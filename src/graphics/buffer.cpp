...
Buffer::Buffer(const vk::DeviceSize buffer_size_bytes,
               const vk::BufferUsageFlags buffer_usage_flags,
               const VmaAllocator allocator,
               const VmaAllocationCreateInfo& allocation_create_info)
    : buffer_size_bytes_{buffer_size_bytes}, allocator_{allocator} {
...
Buffer& Buffer::operator=(Buffer&& buffer) noexcept {
  if (this != &buffer) {
    ...
    buffer_size_bytes_ = std::exchange(buffer.buffer_size_bytes_, 0);
